...
class LlamaAPI:
    ...
    async def _run_stream_for_jupyter(self, api_request_json):
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.hostname}{self.domain_path}",
    ...
    async def run_sync(self, api_request_json):
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.hostname}{self.domain_path}",
                headers=self.headers,
                ...
                return await resp.json()  # assuming server responds with JSON

    async def run(self, api_request_json):
        if api_request_json.get("stream", False):
            return await self.run_stream(api_request_json)
        else:
            return await self.run_sync(api_request_json)
